...
export class GenogramLayout extends go.LayeredDigraphLayout {
    ...
    protected add(net: go.LayeredDigraphNetwork, coll: go.Iterable<go.Part>, nonmemberonly: boolean) {
        var multiSpousePeople = new go.Set() as go.Set<go.Node>;
        // consider all Nodes in the given collection
        var it = coll.iterator;
        while (it.next()) {
        var node = it.value as go.Node;
        ...
        var node = multiSpousePeople.first() as go.Node;
        var cohort = new go.Set() as go.Set<go.Node>;
        this.extendCohort(cohort, node);
        // then encourage them all to be the same generation by connecting them all with a common vertex
        var dummyvert = net.createVertex();
        net.addVertex(dummyvert);
        var marriages = new go.Set() as go.Set<go.Link>;
    ...
    public assignLayers() {
        ...
        var maxsizes = [] as Array<number>;
        this.network.vertexes.each(function(v: go.LayeredDigraphVertex) {
        var lay = v.layer;
